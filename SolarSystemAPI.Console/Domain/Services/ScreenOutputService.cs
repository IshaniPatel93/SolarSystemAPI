...
namespace SolarSystemAPI.ConsoleApp.Domain.Services
{
    public class ScreenOutputService : IOutputService
    {
        ...
        public void OutputAllPlanetsWithAverageMoonTemperatureToConsole()
        {
            var planets = _planetService.GetAllPlanets().ToArray();
            foreach (var planet in planets)
            {
                if (planet.HasMoons())
                {
                    var averageTemperature = planet.AverageMoonTemperature();
                    Console.WriteLine($"Planet {planet.Id} has {planet.Moons.Count} moons. Average Moon Temperature: {averageTemperature}°C");
                }
            }
        }
    }
